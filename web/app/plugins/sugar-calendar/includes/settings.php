...
function sc_settings_page() {
	...
	if ( ( false !== $license ) && empty( $status ) ){?>
		<div class="updated fade"><p class="align-left"><?php _e('Please activate your license key.', 'pippin_sc'); ?></p></div>
	<?php } ?>
	<div class="wrap">
			...
			<table class="form-table">
				...
				<tr valign="top">
					...
					<td>
						...
						<label class="description" for="sc_license_key"><?php _e( 'Please enter and activate your license key in order to get automatic updates and support', 'pippin_sc' ); ?></label>
					</td>
				</tr>
...
function sc_activate_license() {
	...
	if ( isset( $_POST['sc_license_activate'] ) ) {
		...
		// Call the custom API.
		$response = wp_remote_post( 'https://pippinsplugins.com/', array( 'body' => $api_params, 'timeout' => 15, 'sslverify' => false ) );
 ...
 * @return void
 */
function sc_show_license_notice() {

	$is_settings_page = isset( $_GET['page'] ) && 'sc-settings' == $_GET['page'];

	if ( 'expired' === sc_check_license() ) {
		echo '<div class="error info"><p>' . __( 'Your license key for Sugar Event Calendar has expired. Please renew your license to re-enable automatic updates.', 'pippin_sc' ) . '</p></div>';
	} elseif ( 'valid' !== sc_check_license() && ! $is_settings_page ) {
		echo '<div class="notice notice-info"><p>' . sprintf( __( 'Please <a href="%s">enter and activate</a> your license key for Sugar Event Calendar to enable automatic updates.', 'pippin_sc' ), admin_url( 'edit.php?post_type=sc_event&page=sc-settings' ) ) . '</p></div>';
	}
}
add_action( 'admin_notices', 'sc_show_license_notice' );